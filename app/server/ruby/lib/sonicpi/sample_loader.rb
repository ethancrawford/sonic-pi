...
module SonicPi
  class SampleLoader
    ...
    def find_candidates(filts_and_sources, wavetable: false)
      ...
      candidates = extract_candidates(orig_candidates, wavetable: wavetable).dup
      found_proc = false

      if orig_candidates.empty?
        @samples_paths.each do |p|
          if p.end_with?("**")
            candidates.concat(ls_samples(p[0...-2], recursive: true, wavetable: wavetable))
          else
            candidates.concat(ls_samples(p, wavetable: wavetable))
          end
        end
      end

      buffer_type = wavetable ? 'Wavetable' : 'Sample'
      filters_and_procs.each do |f|
        ...
        when NilClass
          # Do nothing
        when Proc
          raise "#{buffer_type} Pack Proc needs to accept 1 argument only. Found #{block.arity}" unless f.arity == 1
          found_proc = true
          candidates = f.call(candidates)
          candidates = [candidates] unless is_list_like?(candidates)
        else
          raise "Unknown #{buffer_type.downcase} filter type: #{f.class} - got: #{f.inspect}"
        end
      end
    ...
    def extract_candidates(candidates, wavetable: false)
      ...
      @cached_extracted_candidates_mutex.synchronize do
        ...
        candidates.each do |c|
          ...
          if expanded.end_with?("**") && File.directory?(expanded[0...-2])
            all_candidates.concat(ls_samples(expanded[0...-2], recursive: true, wavetable: wavetable))
          elsif File.directory?(expanded)
            all_candidates.concat(ls_samples(expanded, wavetable: wavetable))
          elsif File.exist?(expanded)
            all_candidates << expanded
          else
            raise "Unknown #{buffer_type} candidate kind: #{expanded.inspect}. Not a file, directory or /** glob."
          end
    ...
    def ls_samples(path, recursive: false, wavetable: false)
      ...
      @folder_contents_mutex.synchronize do
        res = @cached_folder_contents[path]
        return res if res
        pattern = if wavetable
                    '*.{[wW][aA][vV][eE][tT][aA][bB][lL][eE]}'
                  else
                    '*.{[wW][aA][vV],[wW][aA][vV][eE],[aA][iI][fF],[aA][iI][fF][fF],[fF][lL][aA][cC]}'
                  end
        if recursive
